<script lang="ts">
	...
	import { markAsRead, markAllAsRead, deleteNotification } from '$lib/notifications.remote';
    import { CalendarDate, now, getLocalTimeZone } from '@internationalized/date';
    import { DateFormatter } from '@internationalized/date';

    const formatter = new DateFormatter('en-NG', {
        dateStyle: 'medium',
        timeStyle: 'short'
    });
	...
	async function handleMarkAllAsRead() {
		loading = true;
		try {
			await markAllAsRead(data.user.id);
	...
	function handleNotificationClick(notification: any) {
		...
		if (notification.data?.applicationId) {
			...
			window.location.href = `/app/placements/${notification.data.placementId}`;
		} else if (notification.data?.conversationId) {
			window.location.href = '/app/applications'; // Messages are in applications page
		}
	}
</script>
		...
		<Card>
			<CardContent class="flex flex-col items-center justify-center py-12">
				...
				<p class="text-muted-foreground text-center">
					When you receive applications, matches, or messages, they'll appear here.
				</p>
			</CardContent>
		</Card>
	{:else}
